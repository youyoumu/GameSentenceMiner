...
export class Extension {
    ...
    constructor(options: { name: string; downloadUrl: string | (() => Promise<string>) }) {
    ...
    getExtensionPath() {
        return path.join(app.getPath('userData'), 'extension', this.name);
    }

    getDownloadPath() {
        return path.join(app.getPath('userData'), 'extension', this.name + '.zip');
    }
    ...
    async downloadExtension() {
        ...
        const downloadUrl =
            this.downloadUrl instanceof Function ? await this.downloadUrl() : this.downloadUrl;
